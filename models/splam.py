...
#         # #######################################
#         output = self.fc(self.flatten(self.last_cov(skip)))
#         return output.squeeze(1)

from torch.nn import Module, BatchNorm1d, LeakyReLU, Conv1d, ModuleList, Flatten, Linear, Softmax
import numpy as np
...
class SPLAM(Module):
    def __init__(self, L=64, W=np.array([11]*8+[21]*4+[41]*4), AR=np.array([1]*4+[4]*4+[10]*4+[25]*4),num_classes=3):
        super().__init__()
        self.CL = 2 * (AR * (W - 1)).sum()  # context length
        self.conv1 = Conv1d(4, L, 1)
        self.skip1 = Skip(L)
        ...
        if (len(W) + 1) % 4 != 0:
            self.residual_blocks.append(Skip(L))
        self.last_cov = Conv1d(L, 3, 1)  # Changed to 3 output channels for three classes
        self.flatten = Flatten()
        self.fc = Linear(2403, num_classes)  # Changed to output 3 classes
        self.softmax = Softmax(dim=1)  # Softmax for multi-class classification
    ...
    def forward(self, x):
        ...
        for m in self.residual_blocks:
            x, skip = m(x, skip)
        output = self.fc(self.flatten(self.last_cov(skip)))
        # return self.softmax(output)  # Apply softmax to output probabilities for three classes
        return output  # Apply softmax to output probabilities for three classes
